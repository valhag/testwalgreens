...
--	Stored procedure name: [usp_execute_master_tracker]
...
--	drop PROC [usp_execute_master_tracker]
--*************************************************** CREATING EXECUTE SP *********************************************************************************************

CREATE PROC dbo.usp_execute_master_tracker (
...
BEGIN
	...
	EXEC usp_validate_master_tracker_match 
		@ExecutionID OUT
	    ,@Result OUT
	    ,@FileName OUT
	    ,@CURRENTYEAR OUT
	...
	IF @Result = 0
	BEGIN
		--EXEC usp_generate_master_tracker_match 
