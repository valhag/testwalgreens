--	stored procedure name: usp_generate_SpotRadiokWriter
--	description: usp that generates (insert/update) data of fact_outcome for SPOT RADIO WRITER file type
--	parameters: @executionID just to report success on validation/generation
--	author: HVA
--	Date Creation: 20/11/2018
--	Date modification: 01/23/2019: change stg table dim instead
...
--   Date Modification: 01/30/2019 change input file
...
--   Date Modification: 02/28/2019 SPOT RADIO MEDIA TYPE IS ALWAYS RADIO
--   Date Modification: 02/28/2019 dim_wag_initiative new rows


--	drop PROC usp_generate_SpotRadioWriter

CREATE PROC dbo.usp_generate_SpotRadioWriter (
	@ExecutionID BIGINT
    ,@CURRENTYEAR AS INT
    ,@CURRENTMONTH AS INT) 
AS
	...
	USING
		 (
		  SELECT 
			 ...
			 ,SUM(CAST(srw.billable AS MONEY)) AS value
			 ,dd.id_Dim_Date
			 ,6 AS id_type
		  ...
		  FROM 
			  ...
			  INNER JOIN dbo.month_names AS mn
				  ON mn.month_name = LEFT(srw.month_of_service,3)
			  INNER JOIN dbo.Dim_Date AS dd
				  ON dd.year = CONVERT(INT,'20' + RIGHT(srw.month_of_service,2))
					AND dd.month = mn.id
