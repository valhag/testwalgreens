--	stored procedure name: usp_generate_PrintWriter
--	description: usp that generates (insert/update) data of fact_outcome for PRINT WRITER file type
...
--	drop PROC usp_generate_PrintWriter


CREATE PROC dbo.usp_generate_PrintWriter (
	@ExecutionID BIGINT
    ,@CURRENTYEAR AS INT
    ,@CURRENTMONTH AS INT) 
AS
	...
	USING
		 (
		  SELECT 
			 ...
			 ,CASE
				  WHEN dmt.media_type = 'Magazine' THEN SUM(CAST(srw.billable AS MONEY)) * 1.0225
				  ELSE SUM(CAST(srw.billable AS MONEY))
			  END AS value
			 ,dd.id_Dim_Date
			 ,3 AS id_type
		  ...
		  FROM 
			  ...
			  INNER JOIN dbo.dim_media_type AS dmt
				  ON dmt.media_lawson = RIGHT(srw.gl_code,6)
